...
### API Public url script eval
if [ -n "$ALLURE_DOCKER_PUBLIC_API_URL_IS_SCRIPT" ]; then
    echo "ALLURE_DOCKER_PUBLIC_API_URL is a script, executing it"
    ALLURE_DOCKER_PUBLIC_API_URL=$(eval $ALLURE_DOCKER_PUBLIC_API_URL)
fi
